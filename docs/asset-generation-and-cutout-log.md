...
## 2026-05-29 地图视觉质感 QA

本轮只调整 PixiJS 显示参数，不换图、不压缩、不继续生图：

- 精灵 PNG 按原始宽高比显示，不再强制压成正方形。
- 小屋和精灵合并到同一个演员层，按 y 坐标排序，减少“所有精灵都压在所有小屋上方”的贴纸感。
- 小屋、精灵、岛体阴影改为更宽、更低、更淡，增强贴地感。
- 小屋地垫和等级装饰降低透明度，避免纯色圆点像塑料贴片。
- 成长树常驻光圈和升级粒子降低硬亮度，保留仪式感但减少塑料光效。

验证截图：

```text
/tmp/growth-island-visual-qa-home.png
/tmp/growth-island-visual-qa-focus.png
/tmp/growth-island-visual-qa-xp.png
```
