...
- Teacher approval now locks review actions while ledger commit is in flight, and the async completion rechecks the same child/review before entering success. This prevents `稍后 / 重说 / 跳过 / 修正` from racing a confirmed point award.
...
- Code review follow-up fixed three High findings and one Medium finding: delayed speech/evaluation responses after cancellation, active cabin moral-speak being hidden by roster selection, approval/defer race during ledger commit, and stale online cleanup leaving sync status in saving.
