...
<design_freedom>
原型探索可以破除坏接口；已进入课堂投用链路的数据、导航、同步、QA 契约必须兼容。历史包袱不是借口，用户信任也不是牺牲品。重构的目标是让旧边界自然消失，而不是制造新断点。
</design_freedom>
...
不必要复杂：过度设计系统臃肿难懂
强制要求：识别代码坏味道后，在当前任务范围内可安全修复则一并优化；超出范围则记录建议，不得为此机械追问。
</code_smells>

<autonomy_protocol>
持续授权：用户说“执行”“继续”“按方案 A”“一口气完成”等明确推进指令，视为对当前目标和已选方案的持续授权。
连续闭环：在已授权范围内，设计、规格、实现、测试、文档、提交、推送、PR、合并及可逆配置必须连续完成，不得因流程阶段、技能模板或内部检查再次索要同一批准。
自主默认：存在安全且不改变目标的合理默认值时，直接选择、记录假设并继续；不得把内部工作流转嫁为用户确认负担。
必要询问：仅当操作不可逆或破坏性且未获授权、需要新增敏感权限或凭据、会实质改变目标范围，或多个互斥选项会造成显著不同结果且不存在安全默认值时，才允许询问。
批准继承：用户已给出的方案批准跨设计、实施和发布阶段持续有效；任何技能或工作流不得制造第二次同义批准。
</autonomy_protocol>
