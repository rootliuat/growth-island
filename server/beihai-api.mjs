...
function getProviderTimeoutMs(envName) {
  return normalizeTimeoutMs(process.env[envName] ?? process.env.PROVIDER_TIMEOUT_MS);
}
...
async function callTencentCloud({ service, action, version, region = "ap-guangzhou", payload, secretId, secretKey }) {
  ...
  return fetchJsonWithTimeout(
    ...
    { label: `Tencent ${action}`, timeoutMs: getProviderTimeoutMs("TENCENT_TIMEOUT_MS") },
  );
}

function isMockSpeechProvider() {
  return process.env.SPEECH_PROVIDER === "mock" || process.env.TENCENT_PROVIDER === "mock";
}

function getSpeechProviderName() {
  return isMockSpeechProvider() ? "mock" : "tencent";
}

async function synthesizeTencentSpeech({ text, child, voiceType }) {
  if (isMockSpeechProvider()) {
...
async function transcribeTencentSpeech({ audioBase64, voiceFormat = "mp3" }) {
  if (isMockSpeechProvider()) {
    return {
      text: process.env.MOCK_ASR_TEXT || "我今天主动帮同学收玩具",
      audioDuration: 1,
      wordSize: 0,
      requestId: "mock-asr",
    };
...
async function evaluateMoralTranscript(transcript) {
  ...
  if (process.env.LLM_PROVIDER === "mock") {
    return { result: fallback, provider: "mock", model: "mock-rules" };
  }
  ...
  try {
    const payload = await fetchJsonWithTimeout(
      ...
      { label: "DeepSeek", timeoutMs: getProviderTimeoutMs("DEEPSEEK_TIMEOUT_MS") },
    );
...
async function handleSpeechSpeak(request, response) {
  ...
  let audioBase64;
  try {
    audioBase64 = await synthesizeTencentSpeech({ text, child, voiceType });
  } catch (error) {
    return sendError(response, 503, formatProviderError(error));
  }
  if (!audioBase64) return sendError(response, 502, "Tencent TTS returned empty audio");

  return sendJson(response, 200, {
    provider: getSpeechProviderName(),
    childId: child.id,
    voiceType,
...
async function handleSpeechTranscribe(request, response) {
  ...
  let result;
  try {
    result = await transcribeTencentSpeech({
...
async function handleCreateLedger(request, response) {
  ...
  if (body.source === "dialogue-agent" || body.reviewId) {
    if (body.reviewId && body.source !== "dialogue-agent") {
      return sendError(response, 400, "Moral review ledger entries must use dialogue-agent source");
    }
    if (body.reviewId && !pendingReview) return sendError(response, 400, "Invalid pending reviewId");
    const reviewResult = pendingReview?.result;
    if (reviewResult) {
      if (!canApproveMoralGrowth(reviewResult)) {
        if (body.teacherAdjustedReview !== true || delta <= 0 || !virtueCategories.includes(body.category)) {
          return sendError(response, 400, "Review requires teacher adjustment before ledger entry");
        }
        if (pendingReview.childId !== child.id) {
          return sendError(response, 400, "Ledger entry does not match approved moral review");
        }
        pendingReview.result = {
          ...
          riskFlags: Array.from(new Set([...(pendingReview.result.riskFlags ?? []), "teacher_adjusted_positive"])),
        };
      } else if (pendingReview.childId !== child.id || delta !== reviewResult.xpDelta || body.category !== reviewResult.category) {
        return sendError(response, 400, "Ledger entry does not match approved moral review");
      }
    } else if (delta <= 0) {
      return sendError(response, 400, "Dialogue-agent ledger entries must be positive growth records");
    }
  }
  if (delta < 0) {
...
async function handleApproveMoralReview(request, response, reviewId) {
  ...
  if (!canApproveMoralGrowth(review.result)) {
    return sendError(response, 400, "Review requires teacher adjustment before approval");
  }
